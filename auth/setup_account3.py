"""
Setup script for third Xbox account

This script helps you authenticate a third Xbox account to further increase processing speed.

Usage:
    python setup_account3.py
"""
...
async def main():
    print("=" * 60)
    print("THIRD ACCOUNT SETUP")
    print("=" * 60)
    print()
    print("This will authenticate a third Xbox account.")
    print()
    print("IMPORTANT:")
    print("   - Use a DIFFERENT Microsoft account than accounts 1 and 2")
    ...
    try:
        ...
        print()
        print("Starting authentication for Account 3...")
        print("="*60)
        ...
        # Move the new tokens to account3 file
        if success and os.path.exists("token_cache.json"):
            with open("token_cache.json", 'r') as f:
                account3_tokens = json.load(f)
            
            # Get XUIDs
            xuid_1 = account1_tokens.get('xsts', {}).get('xuid', 'Unknown')
            xuid_3 = account3_tokens.get('xsts', {}).get('xuid', 'Unknown')
            
            # Check if it's the same account
            account1_xsts = account1_tokens.get('xsts', {}).get('token', '')
            account3_xsts = account3_tokens.get('xsts', {}).get('token', '')
            
            if account1_xsts and account1_xsts == account3_xsts:
                print()
                print("=" * 60)
                print("⚠️  ERROR: SAME ACCOUNT DETECTED!")
                print("=" * 60)
                print()
                print(f"You signed in with XUID: {xuid_3}")
                print(f"This is the SAME as Account 1 (XUID: {xuid_1})")
                print()
                print("The tokens have NOT been saved as account 3.")
                print()
            ...
            print(f"Account 3 XUID: {xuid_3}")
            print("=" * 60)
            
            # Save as account3
            with open("token_cache_account3.json", 'w') as f:
                json.dump(account3_tokens, f, indent=2)
            
            print()
            print("Account 3 tokens saved to: token_cache_account3.json")
            ...
            print()
            print("=" * 60)
            print("SUCCESS! You now have 3 accounts configured")
            print("=" * 60)
