"""
Setup script for fourth Xbox account

This script helps you authenticate a fourth Xbox account to further increase processing speed.

Usage:
    python setup_account4.py
"""
...
async def main():
    print("=" * 60)
    print("FOURTH ACCOUNT SETUP")
    print("=" * 60)
    print()
    print("This will authenticate a fourth Xbox account.")
    print()
    print("IMPORTANT:")
    print("   - Use a DIFFERENT Microsoft account than accounts 1, 2, and 3")
    ...
    try:
        ...
        print()
        print("Starting authentication for Account 4...")
        print("="*60)
        ...
        # Move the new tokens to account4 file
        if success and os.path.exists("token_cache.json"):
            with open("token_cache.json", 'r') as f:
                account4_tokens = json.load(f)
            
            # Get XUIDs
            xuid_1 = account1_tokens.get('xsts', {}).get('xuid', 'Unknown')
            xuid_4 = account4_tokens.get('xsts', {}).get('xuid', 'Unknown')
            
            # Check if it's the same account
            account1_xsts = account1_tokens.get('xsts', {}).get('token', '')
            account4_xsts = account4_tokens.get('xsts', {}).get('token', '')
            
            if account1_xsts and account1_xsts == account4_xsts:
                print()
                print("=" * 60)
                print("⚠️  ERROR: SAME ACCOUNT DETECTED!")
                print("=" * 60)
                print()
                print(f"You signed in with XUID: {xuid_4}")
                print(f"This is the SAME as Account 1 (XUID: {xuid_1})")
                print()
                print("The tokens have NOT been saved as account 4.")
                print()
            ...
            print(f"Account 4 XUID: {xuid_4}")
            print("=" * 60)
            
            # Save as account4
            with open("token_cache_account4.json", 'w') as f:
                json.dump(account4_tokens, f, indent=2)
            
            print()
            print("Account 4 tokens saved to: token_cache_account4.json")
            ...
            print()
            print("=" * 60)
            print("SUCCESS! You now have 4 accounts configured")
            print("=" * 60)
